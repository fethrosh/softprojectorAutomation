...
def focus_win ():
    try:
        ...
        try:
            print(YELLOW+"Program not found. Attempting launch"+RE)
            import subprocess
            subprocess.Popen('C:\\Users\\grace\\SoftProjector\\SoftProjector.exe')
            sleep(5)
            focus_win()
        except:
            print(RED+"Unable to open the program"+RE)
...
while(num!='exit'):
    ...
    try:
         x,y=p.locateCenterOnScreen(img3, confidence=.9)
         print(GREEN+"program focused, skipping"+RE)
    except:
         focus_win()
    #main code
    ...
    #runs the database discography adder
        ...
        os.system("shutdown /s /t 1")
        exit()
    # add an option to selct between ann and ann2 without buttons
    else:
        songObject=db.read_library(int(num))
        #error checking
        if (songObject==-1):
            p.alert(text=RED+'Unable to find operation for input. Try again'+RE,title='ERROR')
            num=input()
